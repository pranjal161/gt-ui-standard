...
const DistributorsSearch = ({open, onValidate, onCancel}: DistributorsSearchProps) => {
    ...
    React.useEffect(() => {
        ...
        setObj({search: '', result: {rate: 0, distributor: {}}});
    }, [])
    ...
    return (
        <>
            <Dialog fullWidth={false}
                ...
                actions={<DistributorsSearchActions data={obj} isSearching={isSearching} onSearch={() => setIsSearching(true)} onValidate={() => (onValidate ? onValidate(obj.result) : undefined)} onCancel={onCancel} />}
            />
        </>
    )
}
